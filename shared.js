 ...
 * Contains validation, helpers, and API route factory used by both
 * the Electron main process (main.js) and the standalone web server (server.js).
 */
...
// Write to a temp file then rename, preventing data corruption on crash
...
// IDs are 12-char hex strings generated by crypto.randomBytes(6)
function validateId(id) {
  if (!id || typeof id !== 'string') return false;
  return /^[a-z0-9]{8,20}$/.test(id);
}

// Safely load and parse a JSON file, returning defaultValue on any failure
...
// Trim and truncate user input to prevent oversized data
function sanitizeString(str, maxLength = 1000) {
  if (!str || typeof str !== 'string') return '';
  return str.trim().substring(0, maxLength);
}

// Generate a 12-character hex ID using cryptographically secure random bytes
function generateId() {
...
// Validate person fields: name required, role/team optional, all capped at 200 chars
...
// Validate note fields: content required, optional title and tags array
...
// Validate questions: must be a non-empty array of strings, max 500 items
 ...
 * @param {function} [options.loadSettings] - Optional function to load app settings
 * @param {function} [options.saveSettings] - Optional function to save app settings
 */
function createApiRoutes(expressApp, options) {
  ...
  // ── Data access helpers ─────────────────────────────────────
  // Each person's notes are stored in a separate file: notes_<id>.json
  ...
  // DELETE person — also removes their associated notes file (cascading delete)
  ...
  // GET all unique tags across all notes (scans every person's notes)
  ...
  // PUT update data location (only available if save/load settings provided)
